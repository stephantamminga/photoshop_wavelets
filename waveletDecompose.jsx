...
function generateWavelets(levelCount, addExtraControls) {
    ...
    var activeLayer = srcDoc.activeLayer;

    // Copy the selected layer
    srcDoc.selection.selectAll();
    try { srcDoc.selection.copy(true); }
    catch (e) {
        try { srcDoc.selection.copy(); }
        catch (e) { alert(e); }
    }

    // Duplicate the document (retains ALL properties: size, mode, profile, bit depth)
    var waveletDoc = srcDoc.duplicate();

    // Fix the filename: strip extension
    var docName = srcDoc.name;
    var baseName = docName.replace(/\.[^\.]+$/, '');
    waveletDoc.name = baseName + "-" + activeLayer.name + "-Wavelet";

    // Create a new empty layer
    var newLayer = waveletDoc.artLayers.add();

    // Delete ALL existing layers (except the new empty layer)
    var layers = waveletDoc.layers;
    for (var i = layers.length - 1; i >= 0; i--) {
        if (layers[i] !== newLayer) {
            try {
                waveletDoc.activeLayer = layers[i];
                waveletDoc.activeLayer.remove();
            } catch (e) {
                // Skip locked or background layers
            }
        }
    }

    // Paste the copied layer into the new empty layer
    waveletDoc.activeLayer = newLayer;
    waveletDoc.paste();
    ...
    var group = waveletDoc.layerSets.add();
    group.name = "Wavelet";

    if (addExtraControls) {
        waveletDoc.artLayers[0].move(group, ElementPlacement.INSIDE);

        for (var i = 0; i < levelCount; i++) {
            var groupScale = group.layerSets.add();
            groupScale.blendMode = BlendMode.LINEARLIGHT;
            groupScale.name = "Scale " + String(levelCount - i);
            waveletDoc.artLayers[0].move(groupScale, ElementPlacement.INSIDE);
